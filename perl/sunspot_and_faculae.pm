...
use spotTools;
use strict;
use Date::Calc qw(Days_in_Month Add_Delta_Days Add_Delta_DHMS);
use Data::Dumper;
use diagnostics;
use warnings;
use Carp;
...
# integer value
# I've put this check in because some lines are shorter and cause problems.
# This test could be more subtle, but I don't have that time right now.
...
sub getDateTime {
    ...
    #print " minute = " . $minute;
    my $tmp = 0;
    #print "trying to convert datetime\n\n";
    #print join",",($year, $month, $day_of_month, $hour, $minute, $tmp);
    #print "\n";

    if ($self->year eq '1878' || $self->year eq '1879' || $self->year eq '1880' || $self->year eq '1881' || $self->year eq '1882' || $self->year eq '1883' || $self->year eq '1884') {
            ($year, $month, $day_of_month, $hour, $minute, $tmp) = 
                Add_Delta_DHMS($year, $month, $day_of_month, $hour, $minute, 0, 0, 12, 0, 0);
    }
    #print join",",($year, $month, $day_of_month, $hour, $minute, $tmp);
    #print "\n";
    my $final = sprintf("%4d-%02d-%02d %02d:%02d",$year, $month, $day_of_month, $hour, $minute);
    #print " final = $final";
    #print "\n";
    #print $self->raw_string;
    #print "\n";
...
sub getStationPosition {
    my $self = shift;
    #print "hellow\n";
    my $statcode = $self->getStation;
    my $date = substr($self->day_of_year,0,3);
    my $year = $self->getYear;
    if ($year > 1949 && $date > 123 && $statcode eq 'G') {
        $statcode = 'G1';
    }
    if ($statcode !~ m/\w/) {
        die $self->getDateTime. "   ". $self->raw_string;
    }

    return $sunspot_tool->get_station_location($self->getStation);
}
