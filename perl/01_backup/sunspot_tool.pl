...
sub Load_Sunspots {
    ...
    meanSunspotNumber(\@sunspot_array);
#    calculateYearlyMeans(\@sunspot_array);
    
#    countSunspotsYear(\@sunspot_array);
    
    #testSunspotForError_lookup(\@error_spots, \@sunspot_array);
    
    #foreach my $error_spot (@error_spots) {
            #   }
#        if ($sunspot->Greenwich_sunspot_group_number eq '194') {
