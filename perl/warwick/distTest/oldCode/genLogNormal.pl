...
my $median = 0;
my $maxValue = 0;
for (my $i = 0.001; $i < $range; $i += $range/$numberOfPoints) {
    my $mean = 1;
    my $stddev = 1;
    my $pi = 3.1415968;
    my $logNValue = exp(- ((log($i/$mean)**2)/2*($stddev**2))) / ($i * $stddev * sqrt(2 * $pi));
    #my $logNValue = exp($i) * exp(-exp($i));
    push (@values, $logNValue);
    if ($maxValue < $logNValue) {
        $maxValue = $logNValue; $median = $i;
    }
    $total += $logNValue*$i;
    $count++;
    print "$i ";
