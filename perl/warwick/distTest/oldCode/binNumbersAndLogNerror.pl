...
#            }
            ...
            print " normalisedError = " . $normalisedError; #sqrt($rawContents)/($numberOfPoints*$binWidth);
            my $LNmean = 1.65;
            my $LNsigma = 2.16;
            ...
            $i += $logNmean;
            if ($i < 0) { print " logNPDF = NaN dataPDF = NaN binError = NaN varianceError = NaN "}
            else {
                my $Px = exp(- ((log($i)**2)/2)) / ($i * sqrt(2 * $pi));
                print " logNPDF = " . -log($Px);
                ...
                if ($j < 0) {
                    ...
                    my $Eminusx = exp(- ((log($j)**2)/2)) / ($j * sqrt(2 * $pi));
                    if ($Eminusx < 0) { print " binErrorneg = 0 "; }
                    else {
                ...
                my $Eplusx = exp(- ((log($j)**2)/2)) / ($j * sqrt(2 * $pi));
            ...
            if ($i < 0) { print " logNPDF = NaN dataPDF = NaN binError = NaN varianceError = NaN "}
                ...
                my $Px = exp(- ((log($i)**2)/2)) / ($i * sqrt(2 * $pi));
                print " logNPDF = " . -log($Px);
                print " dataPDF = " . -log($binContents * 114.21);
