...
if (defined($opts{d})) { $distribution = $opts{d}; }
...
-h      : this text
-m #    : mean of the PDF (default = 1)
-n #    : number of points (default = 1000)
-v #    : variance of the PDF (default = 1)
-r #    : range of the output values (default = 6)
-s #    : start of the range (default = 0)
-d distn' : generate number of given distribution {lognormal|gumbel}
...
if ($distribution eq 'lognormal') {
    my @logNormNumbers = &stdlogNormalNumbers($numberOfPoints, $mean, $stddev, $range, $numberOfBins);
    &numbersOutput(@logNormNumbers);
    &numbersStats(@logNormNumbers);
}
elsif ($distribution eq 'gumbel') {
    my @gumbelNumbers = &stdGumbelNumbers($numberOfPoints, $mean, $stddev, $range, $numberOfBins, $startValue);
    &numbersOutput(@gumbelNumbers);
    &numbersStats(@gumbelNumbers);
}
...
sub numbersStats {
    my @numbers = @_;
    my $mean = Statistics::Basic::Mean->new(\@numbers)->query;
    my $variance = Statistics::Basic::StdDev->new(\@numbers)->query;
    my $mySigma = 0;
    foreach my $number (@numbers) {
        $mySigma += ($number - $mean)**2;
    }
    $mySigma = sqrt(1/(scalar @numbers) * $mySigma);
    print "mean: $mean variance: $variance mySigma = $mySigma\n";
}
...
sub stdlogNormalNumbers {
    my ($numberOfPoints, $mean, $stddev, $range) = @_;
    my @logNormalNumbers = ();
    print "generating std LogNormal Numbers\n";
    print "no of points : $numberOfPoints mean $mean, stdev $stddev, range = $range\n";
    ...
    while ($i < $numberOfPoints) {
        ...
        if ($y < $P_x ) {
            my $normValue = $P_x;
            push(@logNormalNumbers, $x);
            $i++;
        }
    }
    return @logNormalNumbers;
...
sub stdGumbelNumbers {
    my ($numberOfPoints, $mean, $stddev, $range, $numberOfBins, $startValue) = @_;
    my @gumbelNumbers = ();
    print "stdGumbelPDFLinBin\n";
    print "no of points : $numberOfPoints mean $mean, stdev $stddev bins $numberOfBins startvalue $startValue\n";
    my $gumbelPeak = 0.367879441171442;
    my $i = 0;
    while ($i < $numberOfPoints) {
        ...
        if ($y < $P_x ) {
            my $normValue = $P_x;
            push(@gumbelNumbers, $x);
            $i++;
        }
    }
    return @gumbelNumbers;
}
