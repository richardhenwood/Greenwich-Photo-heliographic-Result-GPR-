...
#            }
            ...
            #my $normalisedError =  sqrt($rawContents)/($numberOfPoints*$binWidth);
            #print " BinName = $binPosition ";
            #        print " rawContents = $rawContents ";
            #print " NormContents = $binContents ";
            #print " rawBinWidth = " . $binWidth;
            #print " normalisedError = " . $normalisedError;
            #my $LNmean = 1.65;
            #my $LNsigma = 2.16;
            ...
            #print " numberInBin = 
            # my $Px = exp(-($binPosition - 100.87)/83.52) * exp(-exp(-($binPosition - 100.87)/83.52));
            #my $i = $binPosition;
            ...
            my $pi = 3.1415968;
            #
            # 6 day normalisations values
            #
            my $i = ($binPosition - 100.87)/83.52;
            print " x = $i ";
            $i += $gumbelMean;
            my $Px = exp(-$i) * exp(-exp(-$i));
            print " dataPDF = " . $binContents * 83.52;
            print " gumbelPDF = $Px ";
            print " residual2 = " . ($binContents * 83.52 - $Px)**2;
            #         print " gumbelPDF = " . -log($Px);
            #         print " dataPDF = " . -log($binContents * 83.52);
            #         
            #         my $j = $i - ($binWidth/(83.52*2));
            #         if ($j < 0) {
            #             print " binErrorneg = 0 ";
            #         }
            #         else {
            #             my $Eminusx = exp(-$j) * exp(-exp(-$j));
            #             print " binErrorneg = " . -log($Eminusx);
            #         }
            #         
            #         $j = $i + ($binWidth/(83.52*2));
            #         my $Eplusx = exp(-$j) * exp(-exp(-$j));
            #         print " binErrorpos = " . -log($Eplusx);

            #         my $minusErr = ($binContents * 83.52) - ($normalisedError/2);
            #         if ($minusErr < 0) { print " varianceErrorneg = 0 "; }
            #         else {
            #             print " varianceErrorneg = " . -log($minusErr);
            #         }
            #         print " varianceErrorpos = " . -log(($binContents * 83.52) + ($normalisedError/2));




            #         #
            #         # 7 day normalisations values
            #         #
            #         my $i = ($binPosition - 128.12)/99.64;
            #         $i += $gumbelMean;
            #         my $Px = exp(-$i) * exp(-exp(-$i));
            #         print " gumbelPDF = " . -log($Px);
            #         print " dataPDF = " . -log($binContents * 99.64);
            #         
            #         my $j = $i - ($binWidth/(99.64*2));
            #         my $Eminusx = exp(-$j) * exp(-exp(-$j));
            #         print " binErrorneg = " . -log($Eminusx);
            #         
            #         $j = $i + ($binWidth/(99.64*2));
            #         my $Eplusx = exp(-$j) * exp(-exp(-$j));
            #         print " binErrorpos = " . -log($Eplusx);

            #         my $minusErr = ($binContents * 99.64) - ($normalisedError/2);
            #         print " varianceErrorneg = " . -log($minusErr);
            #         print " varianceErrorpos = " . -log(($binContents * 99.64) + ($normalisedError/2));


            #         #
            #         # 8 day normalisations values
            #         #
            #         my $i = ($binPosition - 114.21)/79.62;
            #         $i += $gumbelMean;
            #         my $Px = exp(-$i) * exp(-exp(-$i));
            #         print " gumbelPDF = " . -log($Px);
            #         print " dataPDF = " . -log($binContents * 79.62);
            #         
            #         my $j = $i - ($binWidth/(79.62*2));
            #         my $Eminusx = exp(-$j) * exp(-exp(-$j));
            #         print " binErrorneg = " . -log($Eminusx);
            #         
            #         $j = $i + ($binWidth/(79.62*2));
            #         my $Eplusx = exp(-$j) * exp(-exp(-$j));
            #         print " binErrorpos = " . -log($Eplusx);

            #         my $minusErr = ($binContents * 79.62) - ($normalisedError/2);
            #         print " varianceErrorneg = " . -log($minusErr);
            #         print " varianceErrorpos = " . -log(($binContents * 79.62) + ($normalisedError/2));


            #my $correctedBinPosition = (($binPosition-100.87)/83.52);
            #print " correctedBinPosition = " . $correctedBinPosition;
            print "\n";
