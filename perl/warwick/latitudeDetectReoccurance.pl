...
if you want to just generate some data around a particular year.
END
    exit 0;
}
...
my $includeKey = 0;
if (defined($opts{'k'})) { $includeKey = 1; }
my $onlyYear = 0;
if (defined($opts{'y'})) { $onlyYear = $opts{'y'}; }
...
sub find_unique_sunspots {
    ...
    foreach my $spotNumber (keys (%sunspots)) {
        ...
        foreach my $obsTime (sort keys %{$sunspots{$spotNumber}}) {
            ...
            ##my @timeArr = split('/-/', $obsTime);
            ##print "year = " . $timeArr[0];
            ##print "\n";
            ##if ($onlyYear && ($onlyYear >= $timeArr[0]-1 && $onlyYear <= $timeArr[0]+1)) {
...
# after all the 'whole lifetime' spots have been removed.
            ...
            foreach my $spot (@spotArray) {
                ...
                #print "\n";
                push(@{$partLifeSpots{$spot->getGroupNumber()}}, $spot->getDateTime() . " " . $spot->getCarringtonLongitude() . " " . $spot->getCorrectedWholeSpotArea() . " " . $spot->getLatitude() . " " . $spot->getLongitude());
            }
        }
        $totalSpots++;
    }
    ...
    if ($includeKey) {
        #print "we are going o include a key as well.\n";
        #print "dateStr = $dateStr\n";
        my @dateArr = split(/-/, $dateStr);
        my $year = $dateArr[0];
        $year++;
        printf("testspot %4d-02-01 12:00 100 500 0 (nearedge = 0) 0\n\n\n", $year);
        printf("testspot %4d-02-01 12:00 150 1000 0 (nearedge = 0) 0\n\n\n", $year);
        printf("testspot %4d-02-01 12:00 200 2000 0 (nearedge = 0) 0\n\n\n", $year);
        printf("testspot %4d-02-01 12:00 250 3000 0 (nearedge = 0) 0\n\n\n", $year);
        printf("testspot %4d-02-01 12:00 300 4000 0 (nearedge = 0) 0\n\n\n", $year);
    }
}
...
sub Load_Sunspot_SpotCentric {
    ...
    while (defined (my $line = <FH>)) {
        ...
        $test_spot->parse_data_into_object($line);
        my $obsTime = $test_spot->getDateTime();
        my @timeArr = split(/-/, $obsTime);
        #print "obsTime = $obsTime\n";
        #print "onlyyear = $onlyYear year = '" . $timeArr[0];
        #print "'\n";
        if (!$onlyYear || ($onlyYear >= $timeArr[0]-1 && $onlyYear <= $timeArr[0]+1)) {
            if ($test_spot->is_spot()) {
                push(@{$sunspot_array{$test_spot->getGroupNumber()}{$test_spot->getDateTime}},
