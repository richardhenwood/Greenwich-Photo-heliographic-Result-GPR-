...
my %opts = ();
getopts("f:n:u", \%opts) ;
my $filterUncertainty = 1;
if (!defined($opts{f})) { usage(); }
if (defined($opts{u})) { $filterUncertainty = 0; }
...
sub printSpotCentricSpots {
    ...
    foreach my $spotNumber (sort numeric keys (%sunspots)) {
        ...
        if ($ageDHMS[0] > 0) {
            ...
            else { #print STDERR "not filtering uncertainty!\n"; 
            }
...
# get the mean of the population;
        ...
        foreach my $maxSize (sort @{$ageGroups{$age}}) {
            ...
            if ($diff >= 1) {
                my $xNormF = floor(&logN($maxSize - $ageMean,$range) * $numberOfAfterBins);
                my $bin = $range**($xNormF / $numberOfAfterBins);
                ...
                my $xNormF = floor(&logN(abs($maxSize - $ageMean),$range) * $numberOfBeforeBins);
                my $bin = $range**($xNormF / $numberOfBeforeBins);
        ...
        PDFNormalisedOutputLogBins(\%spotSizeLogBins, $spotCount, $range, $numberOfAfterBins, $numberOfBeforeBins, $ageMean);
        print "\n\n";
    }
}

sub PDFNormalisedOutputLogBins {
    my ($PDFDistBinsRef, $numberOfPoints, $range, $numberOfAfterBins, $numberOfBeforeBins, $mean) = @_;
...
##
    ...
    $binWidths{-0.5} = 1;
    $binWidths{0.5} = 1;
    for (my $binNumber = 0; $binNumber < $numberOfAfterBins; $binNumber++) {
        my $bin = ($range)**($binNumber / $numberOfAfterBins);
        my $nextBin = ($range)**(($binNumber+1)/$numberOfAfterBins);
        $binWidths{$bin} = $nextBin - $bin;
        if (0) {
            print "$binNumber $numberOfAfterBins $bin ";
            print "binwidth = ($nextBin - $bin) ";
            print "binwidth = " . ($nextBin - $bin);
            print "\n";
        }
    }
    for (my $binNumber = 0; $binNumber < $numberOfBeforeBins; $binNumber++) {
        my $bin = ($range)**($binNumber / $numberOfBeforeBins);
        my $nextBin = ($range)**(($binNumber+1)/$numberOfBeforeBins);
        $binWidths{-$bin} = $nextBin - $bin;
        if (0) {
            print "$binNumber $numberOfBeforeBins $bin ";
            print "binwidth = ($nextBin - $bin) ";
            print "binwidth = " . ($nextBin - $bin);
            print "\n";
        }
...
##
    foreach my $binName (sort numeric keys %PDFDistBins) {
        if (defined($binWidths{$binName})) {
            ...
            print STDERR "age = ?? range = $range, number of before/after bins = $numberOfBeforeBins $numberOfAfterBins\n";
    ...
    foreach my $binName (sort numeric keys %PDFDistBins) {
        if (defined($binWidths{$binName}) && $newstddev != 0) {
            ...
            if ($binName < 0) {
                ...
                #print "binwidth = 0.1 ";# . $binWidth/$newstddev;
            ...
            print STDERR "range = $range, number of bins = $numberOfBeforeBins / $numberOfAfterBins\n";
            print STDERR Dumper %PDFDistBins;
            foreach my $binName (sort numeric keys %binWidths) {
                print STDERR "binname: $binName => " . $binWidths{$binName} . "\n";
            }
