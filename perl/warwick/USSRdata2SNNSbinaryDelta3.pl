...
sub printLinkedSpotsSNNS {
    ...
    my $linkedSpotLongitudeSensitivity = 90;
...
# ideally we want to use the measurement from the central meridian:
                    #my $degreesFromLimb = 90 - $spot->getLongitude;
# however, it is not available, so we use:
...
# we have to use carrington since we don't have a measurement of spot CMD                    
