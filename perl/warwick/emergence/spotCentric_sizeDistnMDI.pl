...
sub usage {
    print STDERR << "EOF";
This program loads a MDI SFC format data into a spot centric hash.
...
example: $0 -f /local/scratch/SFcat/ASCII_sunspot_size_1996730000000_199684235959_7vaORc
EOF
    exit;
}
...
sub printSpotCentricSpots {
    ...
    #printf("boundary info: total=%d regions=%d boundary=%d\n",$totalSpots, $regions, $boundary);
    my $regionCount = 0;
    my $previousRegionCount = 0;
    my @boundarys = (0);
    foreach my $size (sort numeric keys %wholeSizes) {
        printf ("size %f count %d\n", $size, $wholeSizes{$size});
        ...
        #printf( "region count = $regionCount %d %d\n", $regionCount % $boundary, $previousRegionCount % $boundary);
    }
