...
-f name : file containing data, in Greenwich format.
-n #    : normalising factor
-s #    : just process for age = # (default = all ages)
-s (#,#) : just process for age = # (default = all ages)
-i      : ignore age and uncertain viewing all together
-g      : output greenwich format for the spot which pass the test

example: $0 -f /users/rhenwood/ihr/data/greenwich/1938.grp -s 9
EOF
    exit;
}
my %opts = ();
getopts("f:n:s:ig", \%opts) ;
if (!defined($opts{f})) { usage(); }
my $normalisationFactor = 1;
if (defined($opts{n})) { $normalisationFactor = $opts{n}; }
my %singleAge = ();
if (defined($opts{s})) { for (split(/,/,$opts{s})) { $singleAge{$_} = 1; } }
my $ignoreAge = 0;
if (defined($opts{i})) { $ignoreAge = 1; }

my $grnOutput = 0;
if (defined($opts{g})) { $grnOutput = 1; }

#print Dumper %singleAge;
#exit 0;
    ...
    foreach my $age (sort numeric keys %chosenSpots) {
        if (($age < 200) && ($correctionFactor[$age] != 0)) {
            ...
            foreach my $spotNumber (@{$chosenSpots{$age}}) {
                ...
                if (keys %singleAge) {
                    #    print "yes singleage\n";
                    if (exists($singleAge{$ageGroup})) {
                        push(@{$ageGroups{$ageGroup}}, [$spotMaxSize, $spotNumber]);
                    }
                    #else { 
                    #    print "age not found: $ageGroup\n";
                    #}
                }
                else {
                    #print "not singleage\n";
...
# we specify the number of bins before 0 and after;
    ...
    #print "grnOutput = '$grnOutput'\n";
    if ($grnOutput) {
        greenwichOutput(\%ageGroups);
    }
    else {
        rawOutput(\%ageGroups);
        getStats(\%ageGroups);
...
sub rawOutput {
    ...
    foreach my $age (sort numeric keys %ageGroups) {
        my @numbers = @{$ageGroups{$age}};
        foreach my $sizeAndNumber (@numbers) {
            #print Dumper $$sizeAndNumber[0];
            print $$sizeAndNumber[0] . " # age = $age, no = " . $$sizeAndNumber[1] . "\n";
        }
        print "\n\n";
    }
}
sub greenwichOutput {
    ...
    my %ageGroups = %{$ageGroupsRef};
    foreach my $age (sort numeric keys %ageGroups) {
        my @spotNumbers = @{$ageGroups{$age}};
        foreach my $sizeAndNumber (@spotNumbers) {
            my $spotNumber = $$sizeAndNumber[1];
            #print $spotNumber . " # age = $age\n";
            foreach my $obsTime (sort keys %{$sunspots{$spotNumber}}) {
