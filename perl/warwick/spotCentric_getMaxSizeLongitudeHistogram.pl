...
# this script finds out where spots achieve their maximum central meridian distance
#
        ...
        foreach my $obsTime (sort keys %{$sunspots{$spotNumber}}) {
            foreach my $spot (@{$sunspots{$spotNumber}{$obsTime}}) {
                if ($spot->getCorrectedWholeSpotArea > $maxSize) {
                    ...
                    $maxSizeCMD = $spot->getCentralMeridianDistance;
                }
            }
        }
        my $bin = ($binSize/2)+$binSize * floor($maxSizeCMD / $binSize);
        if (!defined($spotMaxLatitude{$bin})) {
            $spotMaxLatitude{$bin} = 0;
        }
        $spotMaxLatitude{$bin}++;
        $totalSpots++;
    }
    for (my $i = -90 + ($binSize/2); $i <= 90; $i = $i + $binSize) {
        if (defined($spotMaxLatitude{$i})) {
                print "longitudeBin = $i wholeSpot = " . $spotMaxLatitude{$i};
                print " normalised " . ($spotMaxLatitude{$i}/$totalSpots);
                print "\n";
            }
            else {
                print "longitudeBin = $i wholeSpot = 0 normalised = 0\n";
            }
...
it finds the central meridian distance where spots achieve their greatest whole spot size
and returns a histogram of binned distances.
