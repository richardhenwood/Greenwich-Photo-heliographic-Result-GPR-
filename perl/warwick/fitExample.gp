...
#    	System: Linux 2.6.16.21-0.25-smp
#    
...
unset grid
set key title ""
set key inside right top vertical Right noreverse noinvert enhanced samplen 4 spacing 1 width 0 height 0 autotitles box linetype -2 linewidth 1.000
unset label
unset arrow
...
f(x) = (1/(sig*(2*pi)**0.5))*exp(-((x-mu)**2)/(2*sig**2))
g(x) = exp(- ((log(x)**2)/2*(sig**2))) / (x * sig * sqrt(2 * pi)) # i think this is the log-N pdf
p(x) = exp(x)
h(x) = exp(-((log(x)**2)/2*(sig**2))) / (x*sig*(2*pi)**0.5)
GNUTERM = "x11"
a = 1.42645095646497e-05
xc = 49.4627279836299
dx = 12.8984208098743
FIT_CONVERGED = 1
FIT_NDF = 7
FIT_STDFIT = 0.0295080442391268
FIT_WSSR = 0.00609507272372786
sig = 2.0
mu = 1.0
plot "< ./genPDFs.pl -n 50 -v 2" using 3:6, h(x)
## fit h(x) "< ./genPDFs.pl -n 10 -v 2"  using 3:6 via sig,mu
#    EOF
