...
# this program compiles our training datafile from rome data and training associations
...
See the relevant webpage for  rome format.
The link candidates format is self documenting.

usage: $0 -f file -t triningdata

-f      : file containing data, in rome format.
-t      : file containing link candidates in our made up format

example: $0 -f /users/rhenwood/ihr/data/rome/all.grp -t ~/ihr/data/reclassification/cycle15latitudeFilteredMulti.txt
EOF
    exit;
}
...
sub printLinkedSpotsSNNS {
    ...
    my $initialSpotLongitudeSensitivity = 20; # lower means more sensitive
    my $initialSpotLatitudeSensitivity = 35;
    my $linkedSpotLongitudeSensitivity = 130;
    my $linkedSpotLatitudeSensitivity = 50;
...
sub Load_Sunspot_SpotCentric {
    ...
    while (defined (my $line = <FH>)) {
        ...
        if (!($line =~ m/^</ || $line =~ m/^\s+/)) {
            my $test_spot = new romeSunspot;
            $test_spot->parse_data_into_object($line);
            if (defined($test_spot->getdayCMcrossing)) {
                print "missing cm crossing";
            }
            if ($test_spot->is_valid()) {
