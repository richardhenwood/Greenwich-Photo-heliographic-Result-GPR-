...
example: $0 -g /users/rhenwood/ihr/data/greenwich/2005all.grp -l /tmp/nnLocal/2005linkedSpots.txt
EOF
    exit;
}
...
sub reclassifySpots {
    ...
    foreach my $spot (sort keys %spotLinks) {
...
#                    print $sunspot->getOrigionalGreenwichFormat;
                    ...
                    #if ($sunspot->hasSuffix) {
                    #    print "found suffix\n"; 
                    #    print $sameSpots{$spot};
                    #    print "\n";
                    #    #print $sunspot->raw_string;
                    #}
                    my $newSpotNo = $sameSpots{$spot};
                    #print "new spot no = $newSpotNo\n";
                    $sunspot->putGroupNumber($newSpotNo);
                    print $sunspot->getOrigionalGreenwichFormat;
...
sub getLinkSets {
    ...
    while (defined (my $line = <FH>)) {
        ...
        #print "line = '$line'";
        if (!defined($spot1) || !defined($spot2)) {
            die "no spot values: line number = $lineNumber, contents = '$line'";
        }
        if ($spot1 !~ m/\d+/ || $spot2 !~ m/\d+/) {
            print "no spot values: line number = $lineNumber, contents = '$line'";
            die "broken line: '$spot1' '$spot2' '$line'";
        }
...
sub orderCronologically { 
    ...
    foreach my $linkedSpot (@linkedSpots) {
        if (defined($sunspots{$linkedSpot})) {
            ...
            #print STDERR "can't find spot $linkedSpot\n";
            die "can't find spot $linkedSpot\n";
            exit 1;
        }
...
sub Load_Sunspot_and_Faculae_DateCentric {
    ...
    while (defined (my $line = <FH>)) {
        ...
        if ($test_spot->is_spot()) {
            push(@{$sunspot_array{$test_spot->getDateTime}{$test_spot->getGroupNumberAugmented()}},
            $test_spot);
        }
