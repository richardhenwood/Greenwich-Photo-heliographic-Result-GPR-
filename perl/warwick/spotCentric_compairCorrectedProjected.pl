...
It outputs spot number and difference between recorded projected area
and calculated projected area (from corrected area values).
    ...
    foreach my $spotNumber (sort numeric keys (%sunspots)) {
        foreach my $obsTime (sort keys %{$sunspots{$spotNumber}}) {
            foreach my $spot (@{$sunspots{$spotNumber}{$obsTime}}) {
                my $pws = $spot->getProjectedWholeSpotArea;
                my $cpws = $spot->getCalculatedProjectedWholeSpotArea;
                my $pu = $spot->getProjectedUmbralArea;
                my $cpu = $spot->getCalculatedProjectedUmbralArea;
                if ($pws - $cpws > 2 || $pu - $cpu > 2) { 
