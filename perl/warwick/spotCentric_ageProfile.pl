...
-s      : number of the spot under consideration.

example: $0 -f /users/rhenwood/ihr/data/greenwich/1929.grp -s 10925
EOF
    exit;
}
my %opts = ();
...
&getSpotSizeProfile($opts{s});

sub getSpotSizeProfile {
    my $spotNumber = shift;
    my @birth = ();
    ...
    foreach my $obsTime (sort keys %{$sunspots{$spotNumber}}) {
        ...
        foreach my $spot (@{$sunspots{$spotNumber}{$obsTime}}) {
                ...
                my @timeNow = split(/[- :]/, $obsTime);
                my @ageDHMS = Delta_DHMS(@birth, 0, @timeNow, 0);

                #print join(', ', @ageDHMS);
                my $hrDiff = $ageDHMS[0] * 24 + $ageDHMS[1] + $ageDHMS[2]/60;
                my $size = $spot->getCorrectedWholeSpotArea;
            if ($spot->getCentralMeridianDistance > 60 ||
                $spot->getCentralMeridianDistance < -60) {
                #print "skipping ";
                #print "$hrDiff size = $size\n";
