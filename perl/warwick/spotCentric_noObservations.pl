...
sub printDateCentricSpots {
    my @testDate = (1874,5,9);
    while ($testDate[0] < 1977) {
        my $testDate = sprintf("%4d-%02d-%02d", $testDate[0], $testDate[1], $testDate[2]);
        #print "testdate = $testDate\n";
        if (!defined($sunspots{$testDate})) {
            print "no data for: $testDate\n";
        }
        @testDate = Add_Delta_Days(@testDate, 1);
    }
