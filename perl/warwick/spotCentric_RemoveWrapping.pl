...
This program identifies spots which wrap around the carring longitude and 
moves them above it. 
...
sub printSpotCentricSpots {
    foreach my $spotNumber (sort numeric keys (%sunspots)) {
        my $minimumLongitude = 360;
        my $maximumLongitude = 0;
        foreach my $obsTime (sort keys %{$sunspots{$spotNumber}}) {
            foreach my $spot (@{$sunspots{$spotNumber}{$obsTime}}) {
                if ($minimumLongitude > $spot->getCarringtonLongitude) {
                    $minimumLongitude = $spot->getCarringtonLongitude;
                }
                if ($maximumLongitude < $spot->getCarringtonLongitude) {
                    $maximumLongitude = $spot->getCarringtonLongitude;
                }
            }
        }
        if ($maximumLongitude - $minimumLongitude > 180) {
            #print "$maximumLongitude - $minimumLongitude  " . ($maximumLongitude - $minimumLongitude);
            #print "\n";
            foreach my $obsTime (sort keys %{$sunspots{$spotNumber}}) {
                foreach my $spot (@{$sunspots{$spotNumber}{$obsTime}}) {
                    if ($spot->getCarringtonLongitude < 180) {
                        $spot->putCarringtonLongitude($spot->getCarringtonLongitude + 360);
                    }
                }
            }
        }
