...
# we specify the number of bins before 0 and after;
    ...
    foreach my $age (sort numeric keys %ageGroups) {
        foreach my $maxSize (sort @{$ageGroups{$age}}) {
            if ($maxSize > $maxSizeRange) { $maxSizeRange = $maxSize; }
        }
    }
    ...
    foreach my $age (sort numeric keys %ageGroups) {
        ...
        print "age $age, range = $maxSizeRange ";
# get the mean of the population;
        ...
        foreach my $maxSize (sort @{$ageGroups{$age}}) {
            ...
            if ($diff >= 1) {
                my $xNormF = floor(&logN($maxSize - $ageMean,$maxSizeRange) * $numberOfAfterBins);
                my $bin = $maxSizeRange**($xNormF / $numberOfAfterBins);
                push(@{$spotSizeLogBins{$bin}}, $maxSize);;
            } elsif ($diff >= -1 && $diff <= 1) {
                push(@{$spotSizeLogBins{0}}, $maxSize);;
            } else {
                my $xNormF = floor(&logN(abs($maxSize - $ageMean),$maxSizeRange) * $numberOfBeforeBins);
                my $bin = $maxSizeRange**($xNormF / $numberOfBeforeBins);
    ...
    PDFNormalisedOutputLogBins(\%spotSizeLogBins, $spotCount, $maxSizeRange, $numberOfAfterBins, $numberOfBeforeBins);
}

sub PDFNormalisedOutputLogBins {
    my ($PDFDistBinsRef, $numberOfPoints, $range, $numberOfAfterBins, $numberOfBeforeBins) = @_;
