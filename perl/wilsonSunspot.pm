...
my %sunspot = (
    ...
    Mt_Wilson_sunspot_group_number => undef,
    ...
    solarCycle => undef,
    yearCMcrossing => undef,
    monthCMcrossing => undef,
    dayCMcrossing => undef
);
...
sub parse_data_into_object {
    ...
    my $century = "19";
    if (substr($data_string,2,2) < 60) {
        $century = 20;
    }
    $self->year($century . substr($data_string,2,2));
    $self->month(substr($data_string,4,2));
    $self->day_of_month(substr($data_string,6,2));
    #$self->time_in_thousandths_of_day(substr($data_string,9,4));
    $self->time_in_UT(substr($data_string,9,4));
    $self->Greenwich_sunspot_group_number(trim(substr($data_string,33,5)));
    $self->Mt_Wilson_sunspot_group_number(trim(substr($data_string,27,5)));

    $self->yearCMcrossing(trim(substr($data_string,53,2)));
    $self->monthCMcrossing(trim(substr($data_string,55,2)));
    $self->dayCMcrossing(trim(substr($data_string,57,4)));
    #$self->Greenwich_sunspot_group_number_suffix(substr($data_string,20,1));
    #$self->Mt_Wilson_magnetic_classification(substr($data_string,20,4));
...
# for some reason, post ~1979 mag classification is recorded using characters.
    ...
    $self->Corrected_whole_spot_area_in_millionths_of_solar_hemisphere(substr($data_string,48,4));
    ...
    #}
    my $heliographicLatitudeSign = " ";
    if (substr($data_string,14,1) eq 'S') {
        $heliographicLatitudeSign = "-";
    }
    $self->Latitude($heliographicLatitudeSign . substr($data_string,15,2));


    my $centralMeridianSign = " ";
    if (substr($data_string,17,1) eq 'W') {
        $centralMeridianSign = "-";
    }
    $self->Central_meridian_distance($centralMeridianSign . substr($data_string,18,2));
...
sub getDateTime {
    my $self = shift;
    my $hour = substr($self->time_in_UT, 0,2);
    my $minute = substr($self->time_in_UT, 2,2);
...
sub getFractionOfDay {
    ...
    my $hour = substr($self->time_in_UT, 0,2);
    my $minute = substr($self->time_in_UT, 2,2);
    my $fraction_of_day = ($hour/24) + ($minute/(24*60));
    #print $self->time_in_UT . " = $hour:$minute " . " $fraction_of_day\n";
...
sub getGroupNumber {
    my $self = shift;
    my $groupNumber = $self->Greenwich_sunspot_group_number;
    #print "groupnumber = '$groupNumber'\n";
    if ($groupNumber eq '') {
        $groupNumber = $self->Mt_Wilson_sunspot_group_number;
    }
    return $groupNumber;
}
sub getNumberSuffix {
...
sub getSolarRadii {
    my $self = shift;
    #print "dist = '". $self->Distance_from_centre_of_solar_disc_in_solar_radii . "'\n";
    if ($self->Distance_from_centre_of_solar_disc_in_solar_radii eq '') {
        my $calculatedDistance = sqrt(($self->Latitude)**2 + ($self->getCentralMeridianDistance)**2);
        return $calculatedDistance/90;
    }
    return $self->Distance_from_centre_of_solar_disc_in_solar_radii;
}
sub getPositionAngle {
    my $self = shift;
    #print "polar angle = '" . $self->Polar_angle_in_deg . "'\n";
...
sub getCarringtonLongitude {
    my $self = shift;
    if ( $self->getCentralMeridianDistance eq '-     ') {
        print STDERR $self->getCentralMeridianDistance. " ";
        print STDERR $self->getDayOfMonth . " ";
        print STDERR $self->getMonth . " ";
        print STDERR $self->getGroupNumber . " ";
        exit 0;
    }
    my $carLong = ($self->getSun_East_Limb() + 90) - $self->getCentralMeridianDistance;
    if ($carLong < 0) {
        $carLong += 360;
    }
    if ($carLong > 360) {
...
sub getProjectedUmbralArea {
    my $self = shift;
    #print "area = '" . $self->Observed_umbral_area_in_millionths_of_solar_disc . "'\n";
    if ($self->Observed_umbral_area_in_millionths_of_solar_disc eq '') {
        return 1;
    }
...
sub getProjectedWholeSpotArea {
    my $self = shift;
    if ($self->Observed_whole_spot_area_in_millionths_of_solar_disc eq '') {
        return 500;
        #return 1000 * $self->getSolarRadii();
    }
    return $self->Observed_whole_spot_area_in_millionths_of_solar_disc;
}
sub getCorrectedUmbralArea {
    my $self = shift;
    if ($self->Corrected_umbral_area_in_millionths_of_solar_hemisphere eq '') {
        return 1;
    }
...
sub getCorrectedWholeSpotArea {
    my $self = shift;
    if ($self->Corrected_whole_spot_area_in_millionths_of_solar_hemisphere eq '    ') {
        return 100;
    }
