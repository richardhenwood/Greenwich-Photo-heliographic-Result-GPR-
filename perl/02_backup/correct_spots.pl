...
use sunspot;
use Data::Dumper;
use Date::Calc qw( Days_in_Year Add_Delta_Days Day_of_Year );
use List::Util qw(sum);
use Math::Trig qw(asin sec rad2deg deg2rad acos pi);
use POSIX qw(ceil floor); 

# this program currently finds heliographic positions for a sample spot for a whole year.
...
sub Load_Sunspots {
    ...
    while (defined(my $line = <STDIN>)) {
		...
		$spot->getHeliographicPosition();
       	calculate_heliographic_rad($spot);
...
sub calculate_heliographic {
    ...
    my $day_of_year = Day_of_Year($spot{year}, $spot{month}, $spot{day_of_month});
	...
	$lamda = -1 * rad2deg(asin($lamda));

    #print "nr " . $spot{year} . "-".trim($spot{month})."-".trim($spot{day_of_month}). " " .$spot{Greenwich_sunspot_group_number} . " (" .  &sf4($phi) . ") " . $spot{Latitude} . " (" . &sf4($lamda) . ") " . $spot{Central_meridian_distance} . " " ;

# richard henwood soln.
    my $cos_roe = 1;
    if (1) {
        my $sqrt_bit = 4 *( (sin(deg2rad($SunB0))**2/cos(deg2rad($SunB0)))*(1-sin(deg2rad($lamda))**2*cos(deg2rad($phi))**2) - sin(deg2rad($lamda))**2*cos(deg2rad($phi))**2 - sin(deg2rad($phi))**2/cos(deg2rad($SunB0)) + 1 ) ;
        $sqrt_bit = (cos(deg2rad($SunB0))/2) * sqrt($sqrt_bit);
        $cos_roe = sin(deg2rad($phi)) * sin(deg2rad($SunB0)) + $sqrt_bit;
        print " " . $roe;
...
# willis soln.
    ...
    if (0) {
        my $sin_ki_plus;
        my $A_ = (sin(deg2rad($phi))**2 - sin(deg2rad($SunB0))**2 - sin(deg2rad($lamda))**2*cos(deg2rad($phi))**2*cos(deg2rad($SunB0))**2)**2 + 4*sin(deg2rad($lamda))**2*sin(deg2rad($phi))**2*cos(deg2rad($phi))**2*cos(deg2rad($SunB0))**2;
        my $B_ = (2*sin(deg2rad($lamda))**2*cos(deg2rad($phi))**2*((sin(deg2rad($phi))**2-sin(deg2rad($SunB0))**2-sin(deg2rad($lamda))**2*cos(deg2rad($phi))**2*cos(deg2rad($SunB0))**2)) - 2*sin(deg2rad($phi))**2*cos(deg2rad($SunB0))**2);
        my $C_ = sin(deg2rad($lamda))**4*cos(deg2rad($phi))**4;
        $sin_ki_plus = (-$B_ + sqrt( $B_**2 - 4*$A_*$C_))/(2*$A_);
        $willis_sin_roe = (sin(deg2rad($lamda))*cos(deg2rad($phi)))/sqrt($sin_ki_plus);
    }

    #print "\n";
}

# this version performs calculations in the radian domain - to avoid lots
# of deg2rad calls.
sub calculate_heliographic_rad {
	my $spotRef = shift;
	my %spot = %{$spotRef};
	my $SunB0 = deg2rad($spot{Sun_B0});
	my $SunL0 = deg2rad($spot{Sun_L0});
	my $Sun_P = deg2rad($spot{Sun_P});
    my $ki = deg2rad($spot{Polar_angle_in_deg});
