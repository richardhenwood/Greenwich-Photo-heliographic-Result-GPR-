...
# this code checks to see if there are multiple spots on one day - with the same group number
# It takes a data set ordered by date, and returns any spots with the same group number
# on the same day.
...
sub Check_Sunspots {
    ...
    while (defined(my $line = <STDIN>)) {
        if (substr($line,0,20) eq substr($previous_line,0,20)) {
            print "Group number used more than once for one day\n";
            print $line;
            print $previous_line;
        }
        $previous_line = $line;
...
#    my $r_ovr_R = $spot{Distance_from_centre_fo_solar_disc_in_solar_radii};
#    my $tollerance = 1;
    ...
    foreach my $day_of_year (1 .. 365) {
        #julian day on 01/01/2001
        my $julian_day = 2451910;
        my $spot = new sunspot;
        my ($SunB0, $SunL0, $Sun_P) = $spot->calc_solar($julian_day + $day_of_year);
...
#        print $spot{year} . "-".trim($spot{month})."-".trim($spot{day_of_month}). " " .$spot{Greenwich_sunspot_group_number} . " $phi " . $spot{Latitude} . " $lamda " . $spot{Carrington_longitude} . "($SunL0 " . $spot{Central_meridian_distance} . ")\n";
#	print $spot{year} . "-".trim($spot{month})."-".trim($spot{day_of_month}). " " .$spot{Greenwich_sunspot_group_number} . " $phi " . $spot{Latitude} . " $lamda " . $spot{Carrington_longitude} . "($SunL0 " . $spot{Central_meridian_distance} . ")\n";
#    $julian_day++;
    }
}
