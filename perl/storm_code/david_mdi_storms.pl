...
my $sunImagesFilename = "/users/rhenwood/ihr/sunspots/perl/storm/MDI_whitelight_removeddark_availability.txt";
...
# finally, iterate through the epoch values printing the result.
                ...
                for (my $k = - $pre; $k <= $post; $k++) {
                        printf "day %2d, ", $k;
                        my $outPath = "/tmp/sundata/david_storms/mdi_notdark/";
...
#                        print Dumper $sunImages{$fileTime}{'MDI_SunImage'};
                        ...
                        my $rotatedStr = "";
                        if (defined(@{$sunImages{$fileTime}{'MDI_SunImage'}}[0])) {
#                            print "\n sunimg = " . $sunImages{$fileTime}{'MDI_SunImage'}[0];
                            $rotatedStr = ".rotated";
                        }
                        my $fileName = "MDI_int_".
                            $fileTime[0].".".$fileTime[1].".".$fileTime[2] .
                            "_".$fileTime[3].":".$fileTime[4].$rotatedStr.".gif";
                        my $outFileName = $fileName;
                        $outFileName =~ s/\./_/g;
                        $outFileName =~ s/\:/_/g;
                        my $wgetCommand = "wget -O $outPath$outFileName http://soi.stanford.edu/production/gif_images/intensitygrams/" .
                            $fileTime[0] . "/" . $monthlist[$fileTime[1] -1] . "//$fileName"; 
                            print "$wgetCommand\n"; 
                              system ($wgetCommand);
                        $some_data = 1;
                        #            print "match storm: " . $storms[$j] . "\n";
                        push(@{$outImages{$storms[$j]}}, $outPath . $outFileName);
                    }
                }
                if (!$some_data) {
...
#        $test_spot->parse_data_into_object($line);
        ...
        $test_spot = $dateTime[2];
        push(@{$sunspot_array{$dateTime[0] . " " . $dateTime[1]}{'MDI_SunImage'}}, $test_spot);
    }
    return %sunspot_array;
}
