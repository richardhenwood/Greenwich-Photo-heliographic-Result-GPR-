...
use sunspot;
use Data::Dumper;
use Date::Calc qw( Days_in_Year Add_Delta_Days Delta_DHMS Add_Delta_DHMS);
...
my $bbsunImagesFilename = "/users/rhenwood/ihr/sunspots/perl/storm/bb_whitelight_availability.txt";
my $MDIsunImagesFilename = "/users/rhenwood/ihr/sunspots/perl/storm/MDI_whitelight_dark_availability.txt";
my $RGOsunImagesFilename = "/users/rhenwood/ihr/sunspots/perl/storm/RGO_mockup_availability.txt";

my @QualityOrder = ("SOHO","BBSO","RGO");
...
my %sunImages = &Load_BBSunImages($bbsunImagesFilename, "BBSO");
#$archiveName = "MDI_sunImage";
%sunImages = (%sunImages, &Load_MDISunImages($MDIsunImagesFilename, "SOHO"));
%sunImages = (%sunImages, &Load_RGOSunImages($RGOsunImagesFilename, "RGO"));

#print Dumper %sunImages;
...
# anticipate the storm event by 18 hours to account for travel from sun to earth.
# add another three hours for good measure.
            @time2 = Add_Delta_DHMS(@time2, 00, -17, -30, 0);
...
#   print "result, previous_result = $result $previous_result";
...
# print $pre + $post spot obs values from this point 
                ...
                print " difference = ";
                print join( ',', Delta_DHMS($storms[$j] =~ /(\d+)-(\d+)-(\d+) (\d+):(\d+)/ ,0, $obs_times[$storm_index] =~ /(\d+)-(\d+)-(\d+) (\d+):(\d+)/ ,0));
                print "\n";
...
#                            print " found k = $k hourdiff = $hour_diff\n";
#                            print " k = $k lower = $lower_bound upper = $upper_bound diff = $hour_diff\n";
                        }
                        else {
#                            print "can't find k = $k hourdiff = $hour_diff\n";
#                            print "k = $k lower = $lower_bound upper = $upper_bound diff = $hour_diff\n";
                        }
                       $k++; 
                    }
...
# finally, iterate through the epoch values printing the result.
                ...
                for (my $k = - $pre; $k <= $post; $k++) {
                    ...
                    if (!exists($epoch{$k})) {
                        ...
                        my $fileTime = sprintf "%4d-%02d-%02d", Add_Delta_Days(@epoch_time[0..2], $k);
                        print "$fileTime, no data\n";
                        push(@{$outImages{$storms[$j]}}, [$outPath . "no_data.png",$fileTime, ""]);
                    }
                    else { 
...
#                        print Dumper $sunImages{$fileTime}{'MDI_SunImage'};
                        ...
                        my $rotatedStr = "";
                        my $fileName = "can't find image!";
                        my $chosenArchiveName = "UNKNOWN!";
                        my $wgetCommand = "";
                        my $outFileName = "";
                        foreach my $archiveName (reverse @QualityOrder) {
                            #    print "checking $archiveName\n";
#                            print Dumper @{$sunImages{$fileTime}{$archiveName}};
                            if (defined(@{$sunImages{$fileTime}{$archiveName}}[0])) {
                                $fileName = @{$sunImages{$fileTime}{$archiveName}}[0]; 
                                $outFileName = $fileName;
                                $chosenArchiveName = $archiveName;
                                if ($archiveName eq "SOHO") {
                                    $outFileName =~ s/\./_/g;
                                    $outFileName =~ s/\:/_/g;
                                    $wgetCommand = "wget -O $outPath$outFileName http://soi.stanford.edu/production/gif_images/intensitygrams/dark/" .
                                        $fileTime[0] . "/" . $monthlist[$fileTime[1] -1] . "//$fileName";
                                }
                                if ($archiveName eq "BBSO") {
                                    $outFileName =~ s/\./_/g;
                                    $outFileName =~ s/\:/_/g;
                                    $outFileName =~ s/__/_/g;
                                    $wgetCommand = "wget -O $outPath$outFileName ftp://ftp.bbso.njit.edu/pub/archive/" .
                                    $fileTime[0] . "/" . $fileTime[1] . "/" . $fileTime[2] . "/" . $fileName; 
                                    $wgetCommand = "";
                                    # push(@noDataAvailable, $fileTime);
                                    #    }
                                }
                                if ($archiveName eq "RGO") {
                                    $wgetCommand = "java .... $fileName";
                                    $wgetCommand = "";
                                }
                                else {
                                    print "Unexpected archive name\n";
                                }
                            }
                        }
                        print " -> " .$fileName;
                        ...
                        push(@{$outImages{$storms[$j]}}, [$outPath . $outFileName . ".png", $fileTime, $chosenArchiveName]);
                        #push(@{$outImages{$storms[$j]}}, ["one", "two"]);
                        print "\n";
                    }
                }
                if (!$some_data) {
...
SUMMARY:    
    if ($i == scalar @obs_times || $j == scalar @storms) {
        ...
        foreach my $stormDate (sort keys %outImages) {
            ...
            print "%";
            $startRowNumber = 1;
            foreach my $imgRef (@{$outImages{$stormDate}}) {
#                print Dumper $imgRef;
                
                my ($imageFile, $fileTime, $imgSrc) =  @{$imgRef};
#                my $imageFile = $imageFile[0];
#                my $imageFile = $imgRef;
                #print $fileTime#;
                if ($imgSrc eq 'BBSO') {
                    print "\\includegraphics[trim = 24mm 24mm 24mm 25mm, clip, width=\\imgWidth$imgSrc]{$imageFile}";
                }
                else {
                    print "\\includegraphics[width=\\imgWidth$imgSrc]{$imageFile}";
                }
                if ($startRowNumber % ($numberPerTable + 2) == 0) {
                    print "&\n\\tiny \\begin{sideways} \\captionFont $stormDate \\end{sideways} \\\\\n";
                    #print " &\n";
                }
                else {
                    print " &\n";
                ...
                if ($startRowNumber == $numberPerTable + 3) {
                    #                $dateString .= "\\\\%";
                }
                $dateString .= "\\captionFont $imgSrc " . $fileTime;
                $dateString .= "&\n";
            }
            print substr($dateString,0,-2) . "\\\\\n";
        }
        }
        #  print join "\n", @noDataAvailable;
        exit 0; 
...
# this code loads sun image availability from the BB archive
sub Load_BBSunImages {
    my $filename = shift;
    ...
    while (defined (my $line = <FH>)) {
#        my $test_spot = new sunspot;
        my $test_spot = "The image is available at $archiveName";
#        $test_spot->parse_data_into_object($line);
...
sub Load_MDISunImages {
    ...
    while (defined (my $line = <FH>)) {
        #        my $test_spot = new sunspot;
        my $test_spot = "The image is available at MDI archive in stanford\n";
        #        $test_spot->parse_data_into_object($line);
        my @dateTime = split(' ', $line);
        my $rotateStr = "";
        if (defined($dateTime[2])) {
            $rotateStr = "." .$dateTime[2];
        }
        my $date = $dateTime[0];
        #print "date = $date \n";
        $date =~ s/-/./g;
        #print "date = $date \n";
        my $fileName = "MDI_int_".$date."_".$dateTime[1]."".$rotateStr.".gif";
        #     print 'filename str = ' . $fileName;
        $fileName =~ s/\-/_/g;
        $fileName =~ s/__//g;
        push(@{$sunspot_array{$dateTime[0] . " " . $dateTime[1]}{$archiveName}}, $fileName);
    }
    return %sunspot_array;
}
# this code loads sun image availability from the RGO reconstructions archive
sub Load_RGOSunImages {
    my $filename = shift;
    my $archiveName = shift;
    my %sunspot_array = ();
    open (FH, $filename);
    while (defined (my $line = <FH>)) {
        #        my $test_spot = new sunspot;
        my $test_spot = "The image is available reconstructed from RGO data\n";
        #        $test_spot->parse_data_into_object($line);
        my @dateTime = split(' ', $line);
        my $rotateStr = "";
        if (defined($dateTime[2])) {
