...
-c #    : set CMD filter (default = 50)
-p #    : percentage error (default = 0)

example: $0 -f /users/rhenwood/ihr/data/greenwich/1938.grp -s ~/ihr/sunspots/perl/storm/aastar.lst -d ~/ihr/sunspots/perl/storm/semi_diam6.txt
EOF
    exit;
}

my %opts = ();
getopts("c:f:s:d:p:r", \%opts) ;
...
my $percentageError = 0;
if (defined($opts{p})) { $percentageError = $opts{p}; }
...
#            print "all interpolated for: $storm\n";
    ...
    foreach my $storm (sort keys %stormResults) {
        ...
        if (!defined($stormResult{$bestESOResult}{'ESO'})) {
            $stormResult{$bestESOResult}{'ESO'} = 0;
        }
        $stormResult{$bestESOResult}{'ESO'}++;
        my $bestASOResult = &getBestResult($stormResults{$storm}{'ASO'});
        if (!defined($stormResult{$bestESOResult}{'ASO'})) {
            $stormResult{$bestESOResult}{'ASO'} = 0;
        }
        $stormResult{$bestASOResult}{'ASO'}++;
        
            if ($bestESOResult eq 'x') {
            print "storm = $storm \n";
            #print Dumper %stormResults;
            #       print "storm = $storm ";
            #   print "ESOresult = " . $stormResults{$storm}{'ESO'} . " ";
            #       print "bestESO = $bestESOResult ";
            #       print "ASOresult = " . $stormResults{$storm}{'ASO'} . " ";
...
#    print "totalStorm = $stormCount\n";
  #  print Dumper %stormResult;
    ...
    print "cutoff = $CMDfilter *percentage error = $percentageError*\n";
    my @results = qw(S s A a f F x);
    my $expResult = "|*Expert SO*|";
    my $avResult = "|*Average SO*|";
    foreach my $result (@results) {
        if (defined($stormResult{$result}{'ESO'})) {
            $expResult .= sprintf(" %d |", $stormResult{$result}{'ESO'});
        }
        else {
            $expResult .= sprintf(" %d |", 0);
        }
        if (defined($stormResult{$result}{'ASO'})) {
            $avResult .= sprintf(" %d |", $stormResult{$result}{'ASO'});
        }
        else {
            $avResult .= sprintf(" %d |", 0);
        }
    }
    #print "";
        ...
        foreach my $tDayNumber (sort keys %{$aaStormDates{$storm}}) {
            ...
            #    #die "can't find obstime\n";
                ...
                foreach my $spotNumber (sort keys %{$sunspotDates{$obsTime}}) {
                    ...
                    if (!defined($sunSemiDiameter{$obsTime}) || $sunSemiDiameter{$obsTime} == 0) {
                        #print Dumper $sunSemiDiameter{$obsTime};
                        die "should be a value for semidiameter on: '$obsTime'";
                    }
                    my $W_0exp = ((25+25*($percentageError/100))/($sunSemiDiameter{$obsTime}*2))**2 * 1000000;
                    my $W_0avg = ((41+41*($percentageError/100))/($sunSemiDiameter{$obsTime}*2))**2 * 1000000;
                    my $U_0avg = ((15+15*($percentageError/100))/($sunSemiDiameter{$obsTime}*2))**2 * 1000000;
...
##                       
                            if ($spot->getProjectedWholeSpotArea > $W_0exp) {
                                ...
                                #            print $spot->getProjectedWholeSpotArea;
                                #            print " ";
                                #            print $obsTime;
                                #            print " ";
                                #            print $storm;
...
sub Find_Storm_Dates {
    ...
    foreach my $stormTime (sort keys %aaStarStorms) {
        ...
        my $t1TestDate = sprintf("%d-%02d-%02d %02d:%02d", Add_Delta_DHMS(@stormDate, 00, 0, -14, -36, 00));
        ...
        #print "sunspot = " . $spotNumber[0];
        #print Dumper @spotNumber;
        if (defined($spotNumber[0]) && $spotNumber[0] ne '') {
            ...
            # the method has been changed, and so above is commented out.
            # If the test date is before noon, we use t1 -1, otherwise, t1 is testdate.
            #print "t1testdate = ". substr($t1TestDate, 11, 2);
            if (substr($t1TestDate, 11, 2) > 12) {
                #print "afternoon\n";
                $t1Date = substr($t1TestDate, 0, 10);
            }
            else {
...
sub GEN_Sun_SemiDiameter {
    ...
    foreach my $date (sort @dates) {
        ...
        $dateStr = sprintf("%4d-%02d-%02d %02d:%02d", @values);
        print "$dateStr";
        print " | $progSunSD";
        print "\n";
