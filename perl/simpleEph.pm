...
sub calc_LBP {
    my ($self, $y, $m, $d, $h, $min, $s) = @_;
    return $self->calc_solar(calcJD($y,$m,$d,$h,$min,$s)) , &SunSemiDiameter(Day_of_Year($y,$m,$d));
}
...
# this method calculates Julian Day, 
# it gives identical results to 'Astro::Time' for a few 
# values I've tried.
# and i've just added 'hour' 'min' and 'second' resolution.
sub calcJD {
    my ($y, $m, $d, $h, $min, $s) = @_;
    my $day = $d;
    #print Dumper @_;
    ...
    my $a = floor($y/100);
    my $b = 2 - $a + floor($a/4);
    $d += $h/24.0 + $min/(24*60) + $s/(24*60*60); 
    my $jdg = floor(365.25*($y+4716))+floor(30.6001*($m+1))+$d+$b-1524.5;
    #my $jd0 = floor($jdg+0.5)-0.5;
    #print "my julian day = $jdg \n";
    #my $pmjdg = julian_day($y, $m, $d);
    #$pmjdg += $h/24.0 + $min/(24*60) + $s/(24*60*60); 
    #print "pm julian day = $pmjdg \n";
    # my $mjd = mjd2jd(cal2mjd($day, $m, $y, 0 + $h/24.0 + $min/(24*60) + $s/(24*60*60)));
    # print "pm julian day = $mjd \n\n";
...
sub calc_solar {
    ...
    my $t2=$t*$t;
    my $t3=$t*$t2;
    my $L0 = 280.466+36000.770*$t;
    my $M = 357.529+35999.050*$t;
    my $Mr = $M/$radian;
    my $C = (1.915-0.005*$t)*&round($self,sin($Mr))+(0.020)*&round($self,sin(2.0*$Mr));
    my $sunL = $L0+$C;
    my $v = $M+$C;
    my $omega = 125.04-1934.136*$t;
    my $lngtd = $sunL-0.00569-0.00478*&round($self,sin($omega/$radian));
    my $lngtdr = $lngtd/$radian;
    my $diffk = ($lngtdr-$k);           # both in raidans
    my $oblr = (23.439-0.0130*$t)/$radian;
    my $tx = - &round($self,cos($lngtdr))*&round($self,tan($oblr));
    my $ty = - &round($self,cos($diffk))* &round($self,tan($inc));
    my $x = &round($self,atan($tx));
    my $y = &round($self,atan($ty));
    my $Pr = ($x + $y);         #Postion angle
    my $P = $Pr*$radian;
    my $B0r = &round($self,asin( &round($self,sin($diffk))*&round($self,sin($inc))));     #central latitude
    my $B0 = $B0r*$radian;
    my $etay = -&round($self,sin($diffk))*&round($self,cos($inc));
    my $etax = -&round($self,cos($diffk));
    my $eta = (&round($self,atan2($etay,$etax)))*$radian;
...
sub round {
    ...
    my $value = shift;
    my $precision = "%.4f";
    return sprintf($precision, $value);
}
...
#	my $self = shift;
    ...
    #print "\n";
    #return 10;
    return sprintf("%.4f", $SunSemiDiameter[$dayOfYear] / 2.0);
}
