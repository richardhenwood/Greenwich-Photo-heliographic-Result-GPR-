...
# for some reason, post ~1979 mag classification is recorded using characters.
    ...
    $self->Corrected_whole_spot_area_in_millionths_of_solar_hemisphere(substr($data_string,45,5));
    $self->Corrected_umbral_area_in_millionths_of_solar_hemisphere(substr($data_string,39,5));
    $self->Observed_whole_spot_area_in_millionths_of_solar_disc(substr($data_string,33,5));
    #if ($self->Corrected_whole_spot_area_in_millionths_of_solar_hemisphere =~ m/^\D/) {
    #}
    $self->Observed_umbral_area_in_millionths_of_solar_disc(substr($data_string,27,5));
    ...
    #}
    $self->Distance_from_centre_of_solar_disc_in_solar_radii(".".substr($data_string,51,3));
    $self->Polar_angle_in_deg("-".substr($data_string,55,3));
    $self->Carrington_longitude(substr($data_string,59,3));
    $self->Latitude(substr($data_string,63,3));
...
sub getDateTime {
    my $self = shift;
    my $time = $self->time_in_UT; #(substr($self->time_in_thousandths_of_day, 1) / 1000) * 24;
    if ($time =~ m/\s+/) { $self->time_in_UT("1200"); }
    #print "time = $time\n"; 
    my $hour = substr($self->time_in_UT,0,2);
    my $minute = substr($self->time_in_UT,2,2);
    #printf("%4d-%02d-%02d %02d:%02d",$self->year, $self->month, $self->day_of_month, $hour, $minute);
...
sub getHour {
    my $self = shift;
    return substr($self->time_in_UT,0,2);
}
...
sub getCarringtonLongitude {
    my $self = shift;
    if (!($self->Carrington_longitude =~ m/\d+/)) { print Dumper $self; exit 0;}
...
#sub getLongitude {
#    my $self = shift;
#    if (!($self->Central_meridian_distance =~ m/\d+/)) { print Dumper $self; print "cmd = " . $self->Central_meridian_distance . "\n"; exit 0;}
...
#        if ($carringLong > 90) {
#            print "carrington > 90: $carringLong\n";
#        }
        ...
        print "\n";
        print "\n";
        print "returning CMD = " . $self->Central_meridian_distance;
    }
    return $self->Central_meridian_distance;
}
...
sub is_valid {
    my $self = shift;
    if (($self->Carrington_longitude =~ m/\d+/) && ($self->getLatitude() =~ m/\d+/)) {
       return 1;
    }
   #else { print "failing: " . $self->raw_string; exit 0; }
    return 0;
